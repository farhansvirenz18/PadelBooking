...
export async function GET(request) {
  ...
  try {
    ...
    if (table === 'bookings') {
      ...
      query = supabaseServer.from(table).select('*, users(email, first_name, last_name), coaches(name)');
      ...
      query = supabaseServer.from(table).select('*, users(email, first_name, last_name), membership_tiers(name, monthly_price)');
    }
